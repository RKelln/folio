...
def main(argv: list[str] | None = None) -> None:
    ...
    if is_dry_run:
        ...
        if parsed.subcommand == "compile":
            ...
            print("  Resolved sage-wiki config.yaml (from folio.yaml):")
            print("    # models   ← llm.wiki.models + llm.models.{{fast,quality}}")
            print("    # api      ← llm.{{provider,base_url,api_key_env}}")
            print("    # api.extra_params  ← llm.wiki.api_params")
            print("    # extra_params      ← llm.wiki.extra_params")
            print("    # compiler ← wiki.compiler")
            print("    ---")
